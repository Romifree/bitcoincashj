...
- To simplify bitcoincashj programming in future.
...
Since the very first version bitcoincashj has had the concept of "network parameters": a class that wrapped various
...
This proliferation of global variables makes it harder for developers to use multiple instances of bitcoincashj
simultaneously, for example, to do cross-chain trading of different cryptocurrencies against each other.

Finally, several bitcoincashj objects currently need to be plugged together in ways that aren't always obvious for full
...
NetworkParameters appears everywhere in the bitcoincashj API, and so introducing Context will have a major impact on it
